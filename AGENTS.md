# AGENTS.md

本文件是本專案的 AI 協作開發手冊，提供給 Codex / AI agent 與後續維護者使用。任何修改都應遵守這裡的開發原則、驗證流程與 Git 紀錄規範。

## 1. 專案定位

Live Sound Interactive 是包含多個現場音控互動概念的靜態前端專案，目前包含：

- Gain Staging
- EQ Curves

規劃中：

- Dynamic Compression
- Noise Gate

本專案目標是用清楚、可操作、接近現場控台工作流的方式，幫助使用者理解現場音控中的核心概念與訊號變化。每次修改都應先思考：

- 是否讓使用者更容易理解目前功能所涵蓋的音控概念？
- 是否符合現場控台的操作直覺？
- 是否避免誤導使用者理解 Gain、Fader、Meter、EQ curve 或音訊處理參數的角色？
- 是否在桌機、平板與手機上都能順暢操作？

## 2. 專案技術概況

本專案是靜態前端專案，主要由 HTML、CSS、JavaScript ES Modules 組成。

目前不使用：

- React
- Vue
- TypeScript
- Vite
- Bootstrap
- Material Design

除非使用者明確要求，否則不要引入新框架、建置工具或大型依賴。

## 3. 主要檔案職責

- `index.html`：首頁與概念入口。
- `base.css`：全站基礎樣式、reset、body 與通用排版基礎。
- `layout.css`：頁面主版面、header、main grid、footer，以及部分既有 Gain Staging 版面。
- `components.css`：通用 UI 元件與首頁概念入口樣式，並包含部分既有 Gain Staging 元件樣式。
- `responsive.css`：RWD media query、首頁、Gain Staging 與 EQ Curves 的響應式覆寫。
- `modules/gain-staging/index.html`：Gain Staging 執行頁面。
- `detail.css`：Gain Staging Detail Panel、RMS / Peak 參考圖與細節卡片。
- `simulator.css`：Gain Staging Simulator、Knob、Meter、Fader、狀態區與 Simulator 相關控制。
- `data.js`：Gain Staging 樂器 / 聲源資料、meter scale、fader scale 與共用資料工具。
- `icons.js`：Gain Staging 樂器 / 聲源圖示。
- `pflMeter.js`：Gain Staging PFL / Meter 參考圖示與相關繪製邏輯。
- `simulator.js`：Gain Staging Simulator 狀態、互動、meter animation、reset 與輸出計算。
- `script.js`：Gain Staging 頁面初始化、分類篩選、Detail Panel、Drawer、About Modal 與 PFL 參考圖串接。
- `modules/eq-trainer/fundamentals/interactive-eq/index.html`：目前 EQ Curves 執行頁面。
- `eq-trainer.css`：目前 EQ Curves 的主要樣式，並保留部分歷史 EQ 頁面樣式。
- `eqTrainer.js`：目前 EQ Curves 的 UI 與互動入口。
- `eqData.js`：EQ Curves 頻段與文案資料。
- `interactive-eq-graph.js`：EQ 曲線計算與繪製。
- `interactive-eq-knob.js`：EQ 控制旋鈕。
- `interactive-eq-icons.js`：EQ Filter Type 圖示。
- `components/knob.js`：Gain Staging 與 EQ Curves 共用的中立旋鈕工具。
- `README.md`：專案說明與開發指令。
- `ARCHITECTURE.md`：目前產品與技術架構。
- `AGENTS.md`：AI 協作開發手冊，只記錄協作規範，不放一般使用者文件。

現有檔名與路徑中的 `eq-trainer`、`interactive-eq` 是歷史名稱。除非任務明確要求，不要在一般 UI 修改中順便搬移或重新命名。

## 4. 使用者與教學目標

本專案的使用者可能是：

- 初學 live sound / gain staging / EQ 的使用者。
- 需要快速查詢樂器建議 RMS / Peak 的音響工作者。
- 想理解 Gain、Fader、Meter、EQ filter 與曲線變化差異的學習者。
- 在手機或平板上查詢資料或練習互動概念的現場工作者。

教學內容應符合以下原則：

- 用現場音響語境說明，不要過度抽象。
- 避免讓使用者以為 Fader 可以修復 input clip。
- 避免讓使用者以為 Peak、RMS、Headroom 是同一件事。
- 避免讓使用者以為 EQ 曲線只是一張靜態圖，而忽略 Frequency、Gain、Q 與 Filter Type 的互動關係。
- Simulator 與 EQ Curves 的互動應協助理解訊號流與參數效果，而不只是好看。
- About Modal / 使用指南必須與實際功能同步。

## 5. 需求分析原則

收到需求後，先分析：

- 使用者真正想解決的問題是什麼？
- 是否有更好的設計方式？
- 是否會影響既有功能？
- 是否需要同步修改其他模組？
- 是否需要同步更新使用指南？

若需求描述不足，或存在多種合理實作方式，請先提出問題或提供方案比較，不要自行猜測後直接實作。

## 6. 設計建議原則

AI 不應只是完成需求。若發現以下可能改善方向，應主動提出建議：

- 更好的 UI / UX。
- 更好的架構。
- 更好的模組切分。
- 更好的命名。
- 更符合 Live Sound Workflow 的設計。

提出建議時，請清楚說明取捨與影響，但不要直接改寫使用者需求。最終決策權永遠由使用者決定。

## 7. 功能拆分原則

若需求過大，請先拆分成多個可獨立完成的小任務。每個任務都應：

- 可獨立開發。
- 可獨立驗收。
- 可獨立 Commit。
- 可獨立 Review。
- 不影響既有功能。

避免一次修改過多內容。若需求包含多個子功能，請先提出建議的開發順序。

## 8. 長期維護原則

本專案以長期維護為目標。修改程式時請優先考慮：

- 一致性。
- 可讀性。
- 可維護性。
- 可擴充性。

不要為了追求最新語法或流行寫法而重構既有程式。若目前架構合理，請維持既有設計。

穩定性優先於炫技。若新需求會增加未來維護成本，請先提出建議，而不是直接實作。

## 9. 開發原則

- 優先沿用既有命名、結構與設計語言。
- 修改範圍應盡量小，避免無關重構、格式化或資料變更。
- 小步開發，讓每次修改都能獨立驗收。
- 若需求只涉及互動或 UI 微調，不要改動 Gain / Fader / Meter / EQ graph 的核心計算邏輯。
- 不要新增不必要的新面板、教學流程、音訊播放或大型功能。
- 不要為了單次需求建立過度抽象。
- 若工作區含有使用者或其他流程留下的修改，不要覆蓋或還原，需先確認範圍。
- 若需求會影響操作方式，必須同步檢查使用指南。
- 新功能先在 `develop` 完成與驗證，再合併至 `main`。

## 10. UI / UX 設計原則

本專案的 UI 應維持 live sound / digital console 風格。可參考：

- Behringer WING
- Behringer X32
- Behringer X-Touch
- DiGiCo Quantum
- Yamaha DM7
- Avid S6L

避免套用以下風格：

- Material Design
- Bootstrap 預設元件感
- 一般 SaaS 後台風格
- 手機 App 風格
- 過度裝飾性的行銷頁風格

控制元件應像硬體控台上的功能鍵、旋鈕、推桿或 meter：

- Pad 應像 function pad，不像一般網頁 button。
- Knob 應保留旋鈕的角度、LED ring 與拖曳直覺。
- Fader 應保留推桿、scale、cap 與 unity mark 的關係。
- Meter 應強調 RMS / Peak / Clip 狀態，不要只做抽象圖案。
- EQ graph 應清楚呈現頻率響應與控制點，不要變成無法判讀的裝飾線條。
- Hover、pressed、focus-visible 都需可辨識。

Pressed 狀態若模擬硬體按鍵，優先使用 `translateY(1px)` 與陰影變化，不只使用 scale。

## 11. JavaScript 開發規範

本專案使用 ES Modules。修改 JavaScript 時請遵守：

- 不要把所有邏輯塞回 `script.js`。
- Gain Staging 狀態、meter animation、Gain / Fader 操作應留在 `simulator.js`。
- Gain Staging 樂器資料、scale、helper 應留在 `data.js`。
- Gain Staging 圖示資料應留在 `icons.js`。
- PFL / meter 參考圖邏輯應留在 `pflMeter.js`。
- EQ Curves UI 入口應留在 `eqTrainer.js`。
- EQ Curves 曲線計算應留在 `interactive-eq-graph.js`。
- EQ Curves 旋鈕控制應留在 `interactive-eq-knob.js`。
- 事件綁定要避免重複註冊或造成 memory leak。
- 不要留下 `console.log`、debug flag 或臨時測試 code。
- 優先重用既有 helper，不要重寫一套相同邏輯。
- 操作狀態變更後，相關 UI、meter、graph、readout、status message 必須同步更新。

## 12. CSS 開發規範

CSS 已依責任拆分，新增樣式應放在最合適的檔案：

- 全站基礎：`base.css`
- 主版面：`layout.css`
- 通用元件與首頁概念入口：`components.css`
- Gain Staging Detail Panel：`detail.css`
- Gain Staging Simulator：`simulator.css`
- EQ Curves 與歷史 EQ 頁面樣式：`eq-trainer.css`
- RWD override：`responsive.css`

規範：

- 不要把所有新樣式都丟進 `responsive.css`。
- Simulator 專屬樣式應放在 `simulator.css`。
- EQ Curves 專屬樣式應放在 `eq-trainer.css`。
- RWD 才放在 `responsive.css`。
- 避免使用 `!important`。
- 不要新增一次性、難以追蹤的 class。
- 不要建立與既有命名風格衝突的 class。
- 動態尺寸要有穩定約束，避免 hover、active、文字或 icon 造成 layout shift。
- 觸控防干擾樣式只套用在需要的控制元件，不要影響整頁。

## 13. Gain Staging 開發規範

Gain Staging Simulator 是本專案的核心互動功能之一，修改時請特別注意：

- `resetGainToRecommended()` 與 `resetFaderToUnity()` 是既有 reset 入口，新增 reset 操作應重用它們。
- Gain Knob 拖曳、滾輪、鍵盤操作與 reset 操作不可互相干擾。
- Output Fader 拖曳、滾輪、range input 與 reset 操作不可互相干擾。
- Fader 只影響 output，不應修復 preamp input clip。
- Gain / Preamp 影響 input meter 與後續 output meter。
- Meter animation、peak hold、status message 必須和狀態一致。
- 不要改動 Gain、Fader、Meter 演算法，除非需求明確要求。
- 不要改動樂器 RMS / Peak / Headroom 資料，除非需求明確要求。
- 手機 Safari 相關操作需避免不必要的 double tap zoom、文字選取或 touch callout。
- 只在必要元件上套用 `touch-action`、`user-select`、`-webkit-user-select`、`-webkit-touch-callout`。

## 14. EQ Curves 開發規範

EQ Curves 是目前第二個已開放互動功能，修改時請特別注意：

- 不要改動 EQ graph、曲線計算、Filter Type、旋鈕功能或資料，除非需求明確要求。
- `eqTrainer.js` 負責 UI 組裝與互動入口。
- `eqData.js` 負責頻段與文案資料。
- `interactive-eq-graph.js` 負責 EQ 曲線計算與繪製。
- `interactive-eq-knob.js` 負責 EQ 控制旋鈕。
- `interactive-eq-icons.js` 負責 Filter Type 圖示。
- 任何重新命名或搬移 `modules/eq-trainer/fundamentals/interactive-eq/` 都應作為獨立重構任務。

## 15. RWD 與 Touch 規範

若修改涉及 RWD 或觸控操作，需確認：

- 桌機版 layout 正常。
- 平板版 layout 正常。
- 手機版 layout 正常。
- 觸控目標尺寸足夠。
- 觸控操作不依賴 hover。
- 不遮住 Knob / Fader / Meter / EQ graph 的主要操作區。
- 避免 iOS Safari double tap zoom。
- 避免 iOS Safari long press selection / callout。
- 不要用 `user-scalable=no` 解決互動問題。
- 不要用全頁 `touch-action` 粗暴處理局部問題。

建議至少目視檢查：

- iPhone SE 類窄版寬度。
- iPhone 14 Pro Max 類寬手機。
- iPad Mini / iPad 類平板寬度。
- 1440px 左右桌機寬度。

## 16. 使用指南同步規範

以下修改若發生，必須同步檢查 About Modal / 使用指南：

- 新增、移除或改變操作方式。
- Reset 行為改變。
- Gain / Fader / Meter / EQ graph 行為改變。
- Clip / Warning 文案或狀態改變。
- Simulator 或 EQ Curves 流程改變。
- 桌機與手機操作差異改變。

使用指南應說明實際可用操作，不要保留過時、已移除或容易造成瀏覽器手勢衝突的說明。

## 17. AI 協作流程

每次接到任務時，AI agent 應依序進行：

1. 讀懂需求與限制。
2. 檢查目前工作區狀態。
3. 搜尋相關檔案與既有實作。
4. 確認修改範圍。
5. 實作修改。
6. 自我 code review。
7. 執行驗證。
8. 清除臨時檔。
9. 依使用者要求決定是否 commit / push。
10. 回報結果。

若使用者明確要求不要 commit 或不要 push，必須遵守該次需求。

## 18. Code Review 規範

提交前或回報前請自我檢查：

- 是否改到需求外的檔案？
- 是否改動核心演算法或資料？
- 是否有重複邏輯可以重用既有 function？
- 是否留下 magic number，且沒有明確語意？
- 是否留下 `console.log`、debug output 或臨時測試檔？
- 是否破壞既有拖曳、鍵盤、滾輪或觸控操作？
- 是否造成 RWD 版面問題？
- 是否需要更新使用指南？
- 是否影響可及性，例如 `aria-label`、focus-visible？
- 是否影響 GitHub Actions、ESLint、Prettier 設定？

若發現非本次需求但值得改善的問題，先回報或另開獨立修改，不要混進本次修改。

## 19. 驗證流程

每次完成可獨立驗收的修改後，應依需求執行：

```bash
npm run lint
npm run format:check
```

若本次只修改指定 Markdown，可依使用者要求執行指定檔案的 Prettier check。

同時依修改內容確認：

- 網頁可正常載入。
- Console 無 Error。
- 不影響既有功能。
- 若涉及 Simulator，確認 Gain Knob、Output Fader、Meter 與 reset 行為正常。
- 若涉及 EQ Curves，確認 Frequency、Gain、Q、Filter Type、曲線與旋鈕正常。
- 若涉及 RWD，確認桌機、平板、手機版版面正常。
- 若有產生截圖、測試結果或臨時檔，提交前必須清除。

可用本機 PHP server 檢查頁面：

```bash
php -S 127.0.0.1:8000 -t .
```

然後開啟：

```text
http://127.0.0.1:8000/index.html
```

## 20. Git 提交規範

本專案採用「一個功能，一個 Commit」。

每完成一項功能或一個可獨立驗收的修改，若使用者沒有要求暫停提交，應完成完整 Git 提交流程：

1. `git status`
2. 確認沒有不應提交的檔案。
3. `npm run lint`
4. `npm run format:check`
5. `git add` 必要檔案。
6. `git commit`
7. `git push origin main` 或依當前分支策略推送正確目標。
8. 再次確認 working tree clean。

若使用者明確要求不要 commit 或不要 push，該次任務以工作區修改與驗證回報為準。

禁止：

- force push
- 破壞性清除使用者修改
- 未確認範圍時使用 `git add -A`
- 把臨時檔、截圖、debug output 提交進 repo

## 21. Commit Message 規範

Commit message 必須清楚、簡潔且有意義，建議使用 Conventional Commits。

避免使用：

- `update`
- `fix`
- `change`
- `test`
- `123`
- `asdf`

建議格式：

- `feat: 新增 Gain Reset Pad`
- `fix: 修正手機版 Fader 雙擊問題`
- `style: 調整首頁概念入口`
- `docs: 更新產品文件`
- `docs: 優化 AGENTS AI 協作開發手冊`
- `refactor: 改善 Simulator 架構`

請依照本次修改內容選擇最適合的類型與描述。

## 22. 完成後回報格式

若已 commit 並 push，回報需包含：

- Commit SHA
- Commit Message
- 本次修改摘要，約 3 到 10 點
- 修改檔案列表
- Git 統計資訊，例如 files changed、insertions、deletions
- 驗證結果，例如 `npm run lint`、`npm run format:check`、console / smoke check
- Push 狀態
- Working tree clean 狀態

若使用者要求不要 commit / push，回報需包含：

- 修改檔案列表
- 本次修改摘要
- 驗證結果
- `git diff --stat`
- `git status`
- 明確說明沒有 commit、沒有 push

## 23. 禁止事項

- 不要改動與需求無關的資料、演算法或整體架構。
- 不要提交臨時截圖、測試輸出、debug 檔案或本機環境檔。
- 不要在未確認範圍時使用 `git add -A` 提交混雜工作區。
- 不要使用破壞性 Git 指令清除使用者修改。
- 不要新增大型依賴或工具設定，除非需求明確要求且已確認。
- 不要把 UI 改成 Material Design、Bootstrap 預設或一般 App 風格。
- 不要把觸控問題用全頁禁止縮放或全頁 touch handling 粗暴解決。
- 不要在沒有需求時修改 `package.json`、GitHub Actions 或格式化設定。
